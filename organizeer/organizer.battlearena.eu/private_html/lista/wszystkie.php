<?php
	...
	{
		if($sid == 1)
			$query = "SELECT t.*, u.email, u.username FROM tematy t left join users  u on t.osoba_kontaktowa = u.id ORDER BY t.id DESC";
		else
			$query = "SELECT t.*, u.email, u.username FROM tematy t left join users  u on t.osoba_kontaktowa = u.id WHERE osoba_kontaktowa = '$uid' ORDER BY t.id DESC";
	}
	else
	{
		if($sid == 1)
	...
	<div id="content">
		<?php
			...
			foreach($rows as $row)
			{
				...
				echo $row['data'];
				echo '</td><td>';
				echo '<a href="?id=temat&strona=zobacz&nr_tematu='.$row['id'].'">Podgląd</a>';
				echo '</td></tr>';
